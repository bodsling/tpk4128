...
void *eat(void *argc)
{
    ...
    while (1)
    {
        ...
        pthread_mutex_lock(&waiter); //Asking permission from the waiter to pick up both forks
        pthread_mutex_lock(&lock[leftFork]); //Picking up left fork
        pthread_mutex_lock(&lock[rightFork]);//Picking up right fork
        pthread_mutex_unlock(&waiter); //Done picking up both forks, waiter may move on
